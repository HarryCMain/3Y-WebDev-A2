...
function Form(formtohandle) {
	...
	***************************/	
	Form.prototype.checkUser=function() {
		...
		this.XHR.open("POST","php/checkuser.php",true);
	...
	***************************/	
	TextInput.prototype.doFeedback=function(checkstatus,failmessage) {
		...
		if(checkstatus) {
			...
			if(!fbtarget) {
				var fbtarget=document.createElement("span");
				fbtarget.setAttribute("id",this.fbid);
				this.hte.insertAdjacentElement('afterend',fbtarget);
	...
	***************************/	
	PassInput.prototype.doStrFeedback=function() {
		...
		if(!fbtarget) {
			...
			this.hte.insertAdjacentElement('afterend',fb);
	...
	***************************/	
	DateInput.prototype.check=function() {
		...
		if(checkstatus) {
			...
			checkstatus=age>=16;
		}		this.doFeedback(checkstatus,"Invalid Date or under 16");
		return checkstatus;
	};
